...
export function invokeM5CModelBroker({ request, credentialFd, auditFd, credentialRef = "external-file:deepseek/m5c-role" }, {
  ...
  if (auditFd !== undefined && (!Number.isSafeInteger(auditFd) || auditFd < 0 || auditFd === credentialFd)) throw new TypeError("M5C model broker audit descriptor is invalid");
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [entry instanceof URL ? entry.pathname : entry], { cwd: tmpdir(), shell: false,
      ...
      stdio: ["pipe", "pipe", "ignore", credentialFd, auditFd === undefined ? "ignore" : auditFd] });
    ...
    child.stdin.on("error", () => {}); child.stdin.end(JSON.stringify({ request, credentialRef, auditEnabled: auditFd !== undefined }));
  });
}
