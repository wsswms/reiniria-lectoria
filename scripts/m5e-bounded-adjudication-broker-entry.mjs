...
import {
  invokeM5ECandidateAdjudicationDeepSeek,
  invokeM5EGoalConsolidationDeepSeek,
} from "./m5e-bounded-adjudication-deepseek.mjs";
...
try {
  const envelope = JSON.parse(await readInput(8 * 1024 * 1024)); const stage = envelope?.request?.stage;
  if (!envelope || Object.keys(envelope).sort().join(",") !== "auditEnabled,credentialRef,request"
    ...
    || !["candidate-adjudication", "goal-consolidation"].includes(stage)) throw Object.assign(new Error(), { category: "policy" });
  const invoke = stage === "candidate-adjudication" ? invokeM5ECandidateAdjudicationDeepSeek : invokeM5EGoalConsolidationDeepSeek;
