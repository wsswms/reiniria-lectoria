...
const safe = (value) => {
  ...
  if (output.length < 1 || output.length > 120) throw new Error("P1-Lite audit label is invalid"); return output;
};

async function privateDirectory(path) {
  await mkdir(path, { recursive: false, mode: 0o700 }); const stat = await lstat(path);
  if (!stat.isDirectory() || stat.isSymbolicLink() || stat.uid !== process.getuid() || (stat.mode & 0o077) !== 0) throw new Error("P1-Lite calls directory is invalid");
}

export class M5EP1LiteAuditSession {
  static async create(root) {
    const callsRoot = join(root, "llm-calls"); await privateDirectory(callsRoot); const value = new M5EP1LiteAuditSession(root, callsRoot); await value.writeManifest(); return value;
  }
  constructor(root, callsRoot) { this.root = root; this.callsRoot = callsRoot; this.entries = []; this.sequence = 0; }
  async writeManifest() {
    const value = { schemaVersion: "m5e-p1-lite-llm-audit-manifest-v1", dataClass: "user-provided-public-articles-and-provider-content",
  ...
  async invoke(label, metadata, operation) {
    ...
    try {
      ...
      if (requests.length < 1 || requests.length > 2 || requests.length !== responses.length || !requests[0]?.request?.body || !final) throw new Error("P1-Lite audit event sequence is incomplete");
      ...
      entry.outputDigest = digest(final.response?.rawBody ?? ""); entry.responseBytes = final.response?.bodyBytes ?? null; entry.finishReason = final.response?.finishReason ?? null;
      entry.usage = responses.map((event) => event.response?.usage ?? null); entry.elapsedMs = responses.reduce((sum, event) => sum + (event.elapsedMs ?? 0), 0);
      entry.normalized = final.outcome?.normalized === true;
