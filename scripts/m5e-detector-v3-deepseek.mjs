...
function payload(value, coverage) {
  ...
  try { return Object.freeze({ responseId: value.id, ...normalizeDetectorV3Payload(decoded, coverage) }); }
  catch (error) {
...
export async function invokeM5EDetectorV3DeepSeek(input, { credential, fetchImpl = globalThis.fetch, audit, signal } = {}) {
  ...
  const baseBody = buildDetectorV3DeepSeekBody(input); let priorUsage = null; let repairCode = null;
  for (let attempt = 1; attempt <= input.maximumAttempts; attempt += 1) {
    ...
    audit?.(Object.freeze({ schemaVersion: "reiniria-llm-call-audit-v1", event: "request", provider: "deepseek", role: "planner-detector-v3",
      thinking: "enabled", attempt, maximumAttempts: input.maximumAttempts, startedAt,
    ...
    try {
      ...
      normalized = payload(raw, input.coverage); observedUsage = usage(raw.usage, Date.now() - started);
    } catch (error) {
      ...
      audit?.(Object.freeze({ schemaVersion: "reiniria-llm-call-audit-v1", event: "response", provider: "deepseek", role: "planner-detector-v3",
        thinking: "enabled", attempt, maximumAttempts: input.maximumAttempts, startedAt, completedAt: new Date(completed).toISOString(), elapsedMs: completed - started,
