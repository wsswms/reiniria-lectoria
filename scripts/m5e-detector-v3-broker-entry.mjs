...
import { invokeM5EDetectorV3DeepSeek } from "./m5e-detector-v3-deepseek.mjs";

async function readInput(maximum) {
  const chunks = []; let size = 0; for await (const chunk of process.stdin) {
    size += chunk.length; if (size > maximum) throw new Error(); chunks.push(chunk);
...
try {
  ...
  if (!envelope || Object.keys(envelope).sort().join(",") !== "auditEnabled,credentialRef,request"
    || envelope.credentialRef !== "external-file:deepseek/m5c-role" || envelope.auditEnabled !== true) throw Object.assign(new Error(), { category: "policy" });
  const response = await invokeM5EDetectorV3DeepSeek(envelope.request,
    { credential: credential(3), audit: auditWriterForDescriptor(4) });
