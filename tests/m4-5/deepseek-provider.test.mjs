...
test("DeepSeek request uses fixed endpoint, bearer auth and JSON Object mode with explicit JSON instruction", async () => {
  ...
  assert.match(body.messages[0].content, /kind must be exactly term, entity, fact, relation, or measurement/);
  ...
  assert.match(body.messages[0].content, /ASCII double quote inside a JSON string.*escaped/);
