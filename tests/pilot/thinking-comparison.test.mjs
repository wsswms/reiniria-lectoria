...
import { DEEPSEEK_V4_FLASH_MAX_OUTPUT_TOKENS, buildDeepSeekThinkingComparisonRequest,
...
const request = (thinkingMode) => ({ modelId: "deepseek-v4-flash", questions: ["Question?"],
  ...
  maxOutputTokens: DEEPSEEK_V4_FLASH_MAX_OUTPUT_TOKENS, thinkingMode });
...
test("thinking comparison changes only the thinking switch and sends an exact schema prompt", () => {
  ...
  assert.equal(disabled.body.max_tokens, 384_000);
  assert.match(disabled.body.messages[0].content, /Required shape:/); assert.match(disabled.body.messages[0].content, /byte-for-byte/);
  assert.throws(() => buildDeepSeekThinkingComparisonRequest({ ...request("enabled"), maxOutputTokens: 384_001 }), /comparison request is invalid/);
  assert.throws(() => buildDeepSeekThinkingComparisonRequest({ ...request("enabled"), modelId: "other-model", maxOutputTokens: 2_049 }), /comparison request is invalid/);
});
