...
test("schema 12 fault points leave only retryable v11 or complete v12", async () => {
  for (const point of ["before-migration-12", "after-sql-12", "after-commit-12"]) {
    for (let attempt = 0; attempt < 10; attempt += 1) {
      const root = await mkdtemp(join(tmpdir(), "lectoria-m4-3-migration-"));
      ...
      try {
        ...
        assert.ok([11, 12].includes(database.pragma("user_version", { simple: true })));
