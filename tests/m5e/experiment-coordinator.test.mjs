...
const armResult = (armId) => ({ armId, funnelDigest: digest(armId.toLowerCase()[0]), auditDigest: digest("a"),
  sourceDigest: digest(armId.endsWith("1") ? "1" : "2"), candidateSetDigest: digest(armId.endsWith("1") ? "3" : "4"),
  providerAttempts: 1, braveCalls: 0, fetchUrls: [], qualityArtifactDigest: digest("9") });

function plan() {
  return createM5EExperimentPlan({ part1SourceDigest: digest("1"), part2SourceDigest: digest("2"),
    part1CandidateSetDigest: digest("3"), part2CandidateSetDigest: digest("4"), coldFactSetDigest: digest("0") });
}
...
test("paired arms cannot drift from their frozen source or candidate set", () => {
  const flow = new M5EExperimentCoordinator(plan());
  assert.throws(() => flow.completeArm({ ...armResult("C1"), candidateSetDigest: digest("4") }), /frozen candidate set/);
