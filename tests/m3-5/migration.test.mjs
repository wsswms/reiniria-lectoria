...
test("schema 10 migration fault points leave only retryable v9 or complete v10", async () => {
  for (const point of ["before-migration-10", "after-sql-10", "after-commit-10"]) for (let attempt = 0; attempt < 10; attempt += 1) {
    const root = await mkdtemp(join(tmpdir(), "lectoria-m3-5-migration-"));
    ...
    try {
      ...
      assert.ok([9, 10].includes(version));
      assert.equal(database.prepare("SELECT count(*) AS total FROM sqlite_master WHERE type='table' AND name='export_artifact_metadata'").get().total, version === 10 ? 1 : 0);
