...
const proposals = ref([]); const busy = ref(false); const error = ref(""); const applied = ref(new Set());
...
async function apply(proposal) {
  busy.value = true; error.value = "";
  try { await knowledge.applyProposal(props.workspaceId, proposal.proposalId); applied.value = new Set([...applied.value, proposal.proposalId]); await load(); }
  catch (cause) { error.value = cause.message; } finally { busy.value = false; }
}
onMounted(load);
watch(() => props.workspaceId, () => { applied.value = new Set(); load(); });
</script>
    ...
    <n-list v-else bordered>
      <n-list-item v-for="proposal in proposals" :key="proposal.proposalId">
        <n-space vertical style="width:100%">
          ...
          <n-tag v-if="applied.has(proposal.proposalId) || proposal.application" type="success">已应用到知识库</n-tag>
          <n-tag v-else-if="proposal.current === false" type="error">提案已失效：{{ proposal.staleReason ?? "证据或事实已变化" }}</n-tag>
          <n-space>
            ...
            <n-button v-if="proposal.head.state === 'approved' && proposal.current !== false && !proposal.application && !applied.has(proposal.proposalId)" size="small" type="primary" :loading="busy" @click="apply(proposal)">应用到知识库</n-button>
          </n-space>
        </n-space>
      </n-list-item>
    </n-list>
