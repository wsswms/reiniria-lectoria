...
const bundle = ref(null); const activeId = ref(null); const text = ref(""); const validation = ref(null); const exportRecord = ref(null); const task = ref(null); const error = ref(""); let timer = null;
const active = () => bundle.value?.segments.find((item) => item.segmentId === activeId.value) ?? null;
function choose(item) { activeId.value = item.segmentId; text.value = item.text ?? ""; }
