...
async function restore() { try { await session.get(); loggedIn.value = true; await loadWorkspaces(); } catch (cause) { if (cause.status !== 401) error.value = cause.message; } finally { loading.value = false; } }
async function login() { busy.value = true; error.value = ""; try { await session.login(password.value); password.value = ""; loggedIn.value = true; await loadWorkspaces(); } catch (cause) { error.value = cause.message; } finally { busy.value = false; } }
